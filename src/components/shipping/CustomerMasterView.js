...
const CustomerMasterView = (props) => {
    ...
    <div className="master-view d-flex flex-column h-100">
      <div className="d-flex align-items-center justify-content-between pb-2">
        <div className="d-flex align-items-center gap-3">
          <h1 className="h4 fw-medium text-dark">{rightSidebarData?.[0]?.pagename || 'Customer Management'}</h1>
          <div className="d-flex align-items-center gap-2">
            <span className="badge bg-primary text-white px-2 py-1">
              {props.stats?.totalCustomers || 0} Total Customers
            </span>
            <span className="badge bg-success text-white px-2 py-1">
              {props.stats?.activeCustomers || 0} Active
            </span>
            <span className="badge bg-info text-white px-2 py-1">
              {props.stats?.newCustomers || 0} New This Month
            </span>
            <span className="badge bg-warning text-white px-2 py-1">
              {props.stats?.totalRevenue ? `INR ${props.stats.totalRevenue.toLocaleString()}` : 'INR 0'} Revenue
            </span>
          </div>
        </div>
        <div className="d-flex align-items-center gap-2">
